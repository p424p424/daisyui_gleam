...
fn main_content() -> Element(Msg) {
  html.main(
    ...
    [
      html.div([attribute.class("space-y-6 p-8")], [
        breadcrumbs.new()
          |> breadcrumbs.items([
            breadcrumbs.link(
            ...
            breadcrumbs.item(children: [html.text("Add Document")]),
          ])
          |> breadcrumbs.build,
        breadcrumbs.new()
          |> breadcrumbs.items([
            breadcrumbs.item(children: [html.text("Home")]),
            breadcrumbs.item(children: [html.text("Components")]),
            breadcrumbs.item(children: [html.text("Breadcrumbs")]),
          ])
          |> breadcrumbs.build,
      ]),
    ],
